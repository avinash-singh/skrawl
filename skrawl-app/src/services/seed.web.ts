...
export async function seedIfEmpty(): Promise<void> {
  const db = getDb();
  const count = await db.getFirstAsync('SELECT COUNT(*) as c FROM notes') as { c: number } | null;
  ...
  for (const f of folders) {
    await db.runAsync('INSERT INTO folders', [f.id, f.name, f.context, f.icon, f.color, f.sort]);
  }
  ...
  const notes = [
    ...
    { id: 'n5', title: 'Workout Plan', body: 'Mon: Upper body\nTue: Cardio\nWed: Lower body', context: 'personal', folder_id: 'f2', color: 'orange', type: 'note', priority: 2 },
    { id: 'n6', title: 'Book Recommendations', body: 'The Midnight Library — Matt Haig\nProject Hail Mary — Andy Weir', context: 'personal', folder_id: 'f1', color: 'purple', type: 'note', priority: 3 },
    { id: 'n7', title: 'Weekend Trip Ideas', body: 'Napa Valley, Big Sur, Lake Tahoe', context: 'personal', folder_id: 'f3', color: 'yellow', type: 'note', priority: null },
    { id: 'n8', title: 'Sprint Planning', body: 'Q2: Migrate auth to OAuth 2.1', context: 'business', folder_id: 'f4', color: 'blue', type: 'note', priority: 0, is_pinned: 1 },
  ...
  for (const n of notes) {
    await db.runAsync('INSERT INTO notes', [n.id, n.title, n.body, n.context, n.folder_id, n.color, n.type, n.priority, n.is_pinned || 0]);
  }
  ...
  for (const i of items) {
    await db.runAsync('INSERT INTO checklist_items', [i.id, i.note_id, i.text, i.is_done, i.sort]);
  }
}
